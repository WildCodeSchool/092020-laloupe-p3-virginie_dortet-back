...
router.get("/:id", authenticateWithJsonWebToken, (req, res) => {
  ...
  connection.query(sql, [id], (err, result) => {
    if (err) {
      ...
      res.status(404).json({
        errorMessage: `L'administrateur avec l'id ${id} n'a pas été trouvé`,
      });
    } else {
...
router.post("/", authenticateWithJsonWebToken, (req, res) => {
  ...
  if (!Email || !Password) {
    res
      .status(400)
      .json({ errorMessage: "Merci de renseigner toutes les informations." });
  } else {
    connection.query(
      ...
      (errOne, resultOne) => {
        if (errOne || resultOne.length > 0) {
          ...
          bcrypt.hash(Password, saltRounds, (err, hash) => {
            connection.query(sql, [Email, hash], (errTwo, resultTwo) => {
              if (errTwo) {
                console.log(errTwo);
...
router.post("/login", (req, res) => {
  ...
  if (!Email || !Password) {
    res
      .status(400)
      .json({ errorMessage: "Vous devez fournir toutes les informations." });
  } else {
    // verifying the email
    ...
    connection.query(sql, [Email], (err, result) => {
      if (err) {
        ...
        if (verified) {
          ...
          res
            .status(400)
            .json({ errorMessage: "Le mot de passe est invalide." });
        }
      }
    });
  }
    ...
    bcrypt.hash(Password, saltRounds, (err, hash) => {
      connection.query(sql, [Email, hash, id], (errOne, resultOne) => {
        if (errOne) {
          ...
          res.status(404).json({
            errorMessage: `L'administrateur avec l'${id} n'a pas été trouvé.`,
          });
        } else {
