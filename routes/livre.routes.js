...
router.get("/", (req, res) => {
  ...
  const sql =
    "SELECT B.id AS BookId, B.Title, B.Publication, B.Description, B.Price, B.Link, I.id AS ImageId, I.Image_Name, I.Alt FROM Book as B JOIN Image as I ON B.id=I.Book_id ORDER BY B.id ASC";
...
router.get("/:id", (req, res) => {
  ...
  const sql =
    "SELECT B.id AS BookId, B.Title, B.Publication, B.Description, B.Price, B.Link, I.id AS ImageId, I.Image_Name, I.Alt FROM Book as B JOIN Image as I ON B.id=I.Book_id WHERE B.id=?";
  connection.query(sql, [id], (err, result) => {
    ...
    if (result.length === 0 || result === undefined) {
      ...
      res
        .status(500)
        .send("Une erreur s'est produite lors de la récupération du livre.");
    } else {
      // Displaying the book
      res.status(200).json(books);
    }
...
router.post("/", authenticateWithJsonWebToken, (req, res) => {
  ...
  connection.query(
    sql,
    [Title, Description, Price, Link, Publication],
    (err, result) => {
      if (err) {
        res.status(500).json({ errorMessage: err.message });
      } else {
        ...
        const sql3 =
          "SELECT B.id AS BookId, B.Title, I.id AS ImageId, I.Image_Name, I.Alt FROM Book as B JOIN Image as I ON B.id=I.Book_id WHERE B.id=LAST_INSERT_ID()";
...
router.delete("/:id", authenticateWithJsonWebToken, (req, res) => {
  ...
  connection.query("DELETE FROM Book WHERE id = ?", [idBook], (err) => {
    if (err) {
      res
        .status(500)
        .send("Une erreur s'est produite lors de la suppression du livre.");
    } else {
