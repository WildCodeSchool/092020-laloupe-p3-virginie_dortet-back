...
router.get("/", (req, res) => {
  const sql =
    "SELECT F.id, F.Name, F.Description, F.Link, I.Image_Name, I.Alt FROM Funding AS F JOIN Image AS I ON F.id = I.Funding_id;";
...
router.post("/", authenticateWithJsonWebToken, (req, res) => {
  ...
  connection.query(sql1, [Name, Description, Link], (errOne, resultOne) => {
    if (errOne) {
      ...
      const sql2 =
        "INSERT INTO Image (Image_Name, Alt, Funding_id) VALUES (?, ?, ?)";
  ...
  if (Number.isNaN(parseInt(id, 10))) {
    ...
    connection.query(sql, [id], (err, result) => {
      if (err) {
        ...
        res
          .status(400)
          .json({ errorMessage: `Funding with id ${id} not found.` });
      } else {
